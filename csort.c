...
CSort_load_config(CSort* csort) {
    ...
    if (array_push_from_str(&conf->file_exts, lua, "file_exts") < 0) {
        CSort_panic(csort, "%s, Expected type LUA_TTABLE got %s ???", "file_exts", luaL_typename(lua, -1));
    }
...
//
// CSortEntity
// Is a ctx which holds information about the modules, imports aka parsed content
//
// --------------------------------------------------------------------------------------------
internal CSortModuleObjNode*
CSortEntityFindModule(const CSortEntity* entity, const String_View* tok_view) {
...
internal CSortModuleObjNode*
CSortEntityFindImport(const CSortEntity* entity, const String_View* tok_view) {
    for (CSortModuleObjNode* ptr = entity->modules; ptr; ptr = ptr->next) {
...
internal bool
CSortEntityFindFromImport(CSortModuleObjNode* n, const String_View* tok_view) {
...
//
// Tokenizer
// Our goal is just check identifiers, and not parse ints, strings, bools etc thou we would be checking
// strings. Just check if identifer is a `from` `as` `,` `import`
// 
// --------------------------------------------------------------------------------------------
internal inline enum CSortTokenType
ParseCtxGetIdentifierType(const String_View identifier) {
    if (SV_isEq(identifier, SV("from"))) {
        return CSortTokenFrom;
    } else if (SV_isEq(identifier, SV("import"))) {
...
internal void
ParseCtxEatNextChar(ParseCtx* ctx) {
    if (ctx->next == ctx->end) {
        ctx->ch = -1;
        ctx->curr = ctx->end;
        return;
...
internal void
ParseCtxEatNextCharWithoutIgnores(ParseCtx* ctx) {
    if (ctx->next == ctx->end) {
        ctx->curr = ctx->end;
        ctx->ch = -1;
        return;
    ...
    ctx->ch = *ctx->curr;
    ctx->next += 1;
    if (isspace(ctx->ch)) return ParseCtxEatNextCharWithoutIgnores(ctx);
}

internal bool
ParseCtxIsPossiblySymbol(ParseCtx* ctx) {
    switch (ctx->ch) {
SYMBOLS_STARTS:
        return true;
    }
...
// Calls whenever a newline begins
CSortToken
ParseCtxNextToken(ParseCtx* ctx) {
    ParseCtxEatNextCharWithoutIgnores(ctx);
    char* prev = ctx->curr - 1;
    switch (*prev) {
SYMBOLS_STARTS: {
        while (ctx->ch != -1 && ctx->ch != ' ') {
            ParseCtxEatNextChar(ctx);
        }

        enum CSortTokenType type = ParseCtxGetIdentifierType(SV_slice(prev, ctx->curr));
        }
    }
}
...
//internal CSortModuleObjNode*
//_parse_import_statement_with_duplicate_check(CSortEntity* entity, ParseCtx* parse_info, bool* is_already_kept) {
...
//internal void
//_parse_import_after_from(CSortEntity* entity, ParseCtx* parse_info, CSortModuleObjNode* n) {
//    const CSortToken* tok;
//    do {
//        tok = _update_token(parse_info);
//        if (tok->type != CSortTokenIdentifier) {
...
//    ParseCtx parse_info = ParseCtx_mk(csort, entity);
//    CSortToken tok = {0};
//
//    while (tok = ParseCtxNextToken(&parse_info), tok.type != CSortTokenEof) {
